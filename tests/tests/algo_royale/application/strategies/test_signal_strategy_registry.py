...
from algo_royale.application.strategies.signal_strategy_registry import SignalStrategyRegistry
...
class TestSignalStrategyRegistry:
    ...
    def test_get_combined_weighted_signal_strategy_exception(self, registry, monkeypatch):
        monkeypatch.setattr(registry, '_get_symbol_dir', lambda symbol: (_ for _ in ()).throw(Exception("fail")))
        result = registry.get_combined_weighted_signal_strategy("AAPL")
        assert result is None
    ...
    def test_get_combined_weighted_signal_strategy_unknown_class(self, registry, monkeypatch):
        monkeypatch.setattr(registry, 'symbol_strategy_map', {"AAPL": {"Unknown": {"viability_score": 1, "params": {}}}})
        result = registry.get_combined_weighted_signal_strategy("AAPL")
        assert result is None
