...
class TestTradeService(TestCase):
    ...
    def setUp(self, MockTradesDAO):
        ...
        self.service = TradeRepo(dao=self.mock_dao)
    ...
    def test_get_trades_by_symbol(self):
        self.mock_dao.fetch_trades_by_symbol.return_value = [
            ("mock_trade_1",),
            ("mock_trade_2",),
        ]
    ...
    def test_calculate_trade_pnl(self):
        result = self.service.calculate_trade_pnl(
            entry_price=Decimal("100.00"), exit_price=Decimal("120.00"), shares=5
        )
    ...
    def test_get_trades_by_date_range(self):
        ...
        self.mock_dao.fetch_trades_by_date_range.assert_called_once_with(
            start_date, end_date
        )
        self.assertEqual(result, [("mock_trade",)])
    ...
    def test_get_trades_by_symbol_and_date(self):
        ...
        result = self.service.get_trades_by_symbol_and_date(
            symbol, start_date, end_date
        )
        self.mock_dao.fetch_trades_by_symbol_and_date.assert_called_once_with(
            symbol, start_date, end_date
        )
        self.assertEqual(result, [("mock_trade",)])
