...
class BacktestVisualizer:
    ...
    def _preprocess_data(self):
        ...
        self.data['strategy_returns'] = self.data['signal'].shift(1).eq('buy').astype(int) * self.data['returns']
        ...
        self.data['drawdown_market'] = (self.data['cumulative_market'] - self.data['peak_market']) / self.data['peak_market']
        
        self.data['peak_strategy'] = self.data['cumulative_strategy'].cummax()
        self.data['drawdown_strategy'] = (self.data['cumulative_strategy'] - self.data['peak_strategy']) / self.data['peak_strategy']
    ...
    def performance_metrics(self, strategies=None, symbols=None, risk_free_rate=0.0):
        ...
        for (strategy, symbol), group in grouped:
            ...
            annualized_return = (1 + total_return) ** (252 / (len(group) / (6.5*60))) - 1  # Assuming minute data, 6.5 trading hours/day
            
            returns = group['strategy_returns'].dropna()
            volatility = returns.std() * np.sqrt(252 * 6.5 * 60)  # Annualized volatility for minute data
            sharpe_ratio = (returns.mean() * 252 * 6.5 * 60 - risk_free_rate) / (volatility) if volatility > 0 else 0
    ...
    def plot_metrics_heatmap(self, metric='Total Return (%)', strategies=None, symbols=None, title="Performance Heatmap"):
        """
        ...
        # Pivot for heatmap
        heatmap_data = metrics_df.pivot(index='Strategy', columns='Symbol', values=metric)
