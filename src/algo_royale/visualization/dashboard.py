...
class BacktestDashboard:
    ...
    def _find_result_files(self):
        ...
        if not result_files:
            ...
            print(
                f"Directory structure should be: {self.results_dir}/strategy/symbol/files.csv"
            )
    ...
    def run(self):
        ...
        with st.sidebar:
            ...
            self.selected_strategy = st.selectbox(
                "Select Strategy", options=list(self.file_groups.keys())
            )
            ...
            if self.selected_strategy:
                ...
                if self.selected_symbol:
                    selected_file = self.file_groups[self.selected_strategy][
    ...
    def _display_main_content(self):
        ...
        tab1, tab2, tab3 = st.tabs(
            ["📊 Overview", "📈 Performance", "🔍 Trade Analysis"]
        )
    ...
    def _show_trade_analysis(self):
        ...
        if hasattr(self.visualizer, "data"):
            ...
            symbol = st.selectbox(
                "Select Symbol", self.selected_symbols, key="trade_symbol"
            )

            fig = self.visualizer.plot_trades(symbol=symbol, strategy=strategy, days=7)
            st.plotly_chart(fig, use_container_width=True)
        else:
