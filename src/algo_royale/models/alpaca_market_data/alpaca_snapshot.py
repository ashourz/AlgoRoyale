...
class Trade(BaseModel):
    ...
    timestamp: str = Field(
        ..., alias="t", description="ISO 8601 timestamp of the trade"
    )
    exchange: str = Field(
        ..., alias="x", description="Exchange where the trade occurred"
    )
    ...
    tape: str = Field(
        ..., alias="z", description="SIP tape the trade came from (A, B, or C)"
    )
...
class Quote(BaseModel):
    ...
    timestamp: str = Field(
        ..., alias="t", description="ISO 8601 timestamp of the quote"
    )
...
class Snapshot(BaseModel):
    ...
    def from_raw(cls, raw: dict) -> "Snapshot":
        ...
        return cls(
            latest_trade=Trade.from_raw(raw["latestTrade"])
            if "latestTrade" in raw
            else None,
            latest_quote=Quote.from_raw(raw["latestQuote"])
            if "latestQuote" in raw
            else None,
            ...
            previous_daily_bar=Bar.from_raw(raw["prevDailyBar"])
            if "prevDailyBar" in raw
            else None,
        )
...
class SnapshotsResponse(RootModel[Dict[str, Snapshot]]):
    ...
    def from_raw(cls, raw: dict) -> "SnapshotsResponse":
        ...
        return cls({symbol: Snapshot.from_raw(data) for symbol, data in raw.items()})
