...
class BufferedWickReversalEntryCondition(BaseBufferedCondition):
    """
    Buffered Wick Reversal Entry Condition
    Triggers when lower wick is sufficiently long compared to body.
    """
    ...
    def _evaluate_condition(self) -> bool:
        ...
        for col in [self.lower_wick_col, self.body_col]:
        ...
        if body_safe == 0:
            body_safe = 1e-8
        long_lower_wick = (
            df[self.lower_wick_col].iloc[-1] > self.wick_body_ratio * body_safe
        )
        self.logger.debug(
            f"Lower wick: {df[self.lower_wick_col].iloc[-1]}, Body: {body_safe}, Ratio: {self.wick_body_ratio}, Result: {long_lower_wick}"
        )
