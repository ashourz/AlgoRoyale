...
class BufferedMovingAverageCrossoverExitCondition(BaseBufferedCondition):
    """
    Buffered Moving Average Crossover Exit Condition
    Triggers when short MA crosses below long MA, with trend and volume filters.
    """
    ...
    def _evaluate_condition(self) -> bool:
        ...
        price_below_trend = close.iloc[-1] < trend_ma.iloc[-1]
        volume_below_ma = volume.iloc[-1] < volume_ma.iloc[-1]
        result = prev_cross and curr_cross and price_below_trend and volume_below_ma
        self.logger.debug(
            f"MA crossover exit with trend/volume filter triggered. Result: {result}"
        )
        return result
