...
class MovingAverageCrossoverEntryCondition(StrategyCondition):
    ...
    def optuna_suggest(cls, trial: Trial, prefix: str = ""):
        return cls(
            ...
            short_window=trial.suggest_categorical(
                f"{prefix}short_window", [5, 10, 15, 20]
            ),
            long_window=trial.suggest_categorical(
                f"{prefix}long_window", [30, 50, 100, 200]
            ),
            trend_window=trial.suggest_categorical(
                f"{prefix}trend_window", [100, 200, 300]
            ),
