...
def signal_summary_validator(d, logger: Loggable) -> bool:
    ...
    for metric in ["total_return", "sharpe_ratio", "win_rate", "max_drawdown"]:
        if metric not in d["summary"] or not summary_metric_validator(
            ...
            logger.warning(
                f"Validation failed: Metric '{metric}' missing or invalid in summary. Value: {d['summary'].get(metric)}"
            )
            return False
    ...
    if "most_common_best_params" not in d or not most_common_best_params_validator(
        ...
        logger.warning(
            f"Validation failed: 'most_common_best_params' missing or invalid. Value: {d.get('most_common_best_params')}"
        )
        return False
    ...
    if "window_params" not in d or not window_params_validator(
        ...
        logger.warning(
            f"Validation failed: 'window_params' missing or invalid. Value: {d.get('window_params')}"
        )
        return False
    # Validate strategy
    if "strategy" not in d or not isinstance(d["strategy"], str):
        logger.warning(
            f"Validation failed: 'strategy' missing or not str. Value: {d.get('strategy')}"
        )
        return False
    return True
