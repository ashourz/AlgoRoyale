...
class PortfolioCrossWindowEvaluator:
    ...
    def run(
        ...
        def most_common_param(param_name):
            all_vals = [
                tuple(sorted(wr.get("params", {}).get(param_name, {}).items()))
                for wr in window_results
                if param_name in wr.get("params", {})
            ]
            if not all_vals:
                return None
            return dict(Counter(all_vals).most_common(1)[0][0])
        ...
        out_path = self.strategy_dir / self.output_filename
