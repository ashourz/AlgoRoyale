...
class StageDataLoader:
    def __init__(self, config: Config, logger: Logger, pipeline_data_manager: PipelineDataManager):
        try:
    ...
    async def load_all_stage_data(self, stage: PipelineStage) -> Dict[str, Callable[[], AsyncIterator[pd.DataFrame]]]:
        ...
        for symbol in self.watchlist:
            try:
                ...
                data[symbol] = lambda s=symbol, st=stage: self.load_symbol(stage=st, symbol=s)
                self.logger.info(f"Prepared async data loader for: {stage} | {symbol}")
            except Exception as e:
                self.logger.error(f"Failed to prepare loader for {stage} | {symbol}: {str(e)}")
    ...
    async def load_symbol(self, stage: PipelineStage, symbol: str) -> AsyncIterator[pd.DataFrame]:
        """Async generator yielding DataFrames, fetching data if needed"""
        symbol_dir = self._get_data_ingest_stage_symbol_dir(symbol = symbol)
        ...
        async for df in self._stream_existing_data_async(stage=stage, symbol_dir=symbol_dir):
            yield df

    def _get_data_ingest_stage_symbol_dir(self, stage:PipelineStage, symbol: str) -> Path:
    ...
    async def _stream_existing_data_async(self, stage:PipelineStage, symbol_dir: Path) -> AsyncIterator[pd.DataFrame]:
