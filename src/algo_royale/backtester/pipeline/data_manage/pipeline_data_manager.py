...
class PipelineDataManager:
    ...
    def get_directory_path(self, stage: PipelineStage, strategy_name: Optional[str], symbol: str) -> Path:
        if strategy_name:
    ...
    def is_stage_done(self, stage: PipelineStage) -> bool:
        done_file = self.get_stage_path(stage) / f"{stage.value}{DataExtension.DONE.value}"
        return done_file.exists()
    
    def is_symbol_stage_done(self, stage: PipelineStage, strategy_name: Optional[str], symbol: str) -> bool:
        if strategy_name:
            done_file = self.get_directory_path(stage, strategy_name, symbol) / f"{stage.value}{DataExtension.DONE.value}"
        else:
            # If no strategy name is provided, use the symbol as the directory
            done_file = self.get_directory_path(stage, None, symbol) / f"{stage.value}{DataExtension.DONE.value}"
        return done_file.exists()
    
    def mark_stage(self, stage: PipelineStage, statusExtension: DataExtension) -> None:
    ...
    def mark_symbol_stage(self, stage: PipelineStage, strategy_name: Optional[str], symbol: str, statusExtension: DataExtension) -> None:
        if strategy_name:
    ...
    def write_error_file(self, stage: PipelineStage, strategy_name: Optional[str], symbol: str, filename: str, error_message: str) -> None:
        """
