...
class OrderGenerator(SymbolSubscriberManager):
    ...
    def _can_subscribe(self, symbol):
        return super()._can_subscribe(symbol)

    async def _async_subscribe_to_order_events(
        self,
        symbols: list[str],
    ...
    async def _async_unsubscribe_from_order_events(
    ...
    async def _async_restart_stream(self, symbols: list[str]) -> list[str]:
        """
